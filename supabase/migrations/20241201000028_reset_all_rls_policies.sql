-- 모든 RLS 정책을 제거하고 새로 설정
-- 기존 정책과의 충돌을 방지하기 위해 모든 정책을 제거 후 재생성

-- 1. reservations 테이블 RLS 정책 초기화 및 재설정
...
-- 2. work_records 테이블 RLS 정책 초기화 및 재설정
DROP POLICY IF EXISTS "Allow all operations on work_records" ON public.work_records;
DROP POLICY IF EXISTS "work_records_public_select" ON public.work_records;
DROP POLICY IF EXISTS "work_records_admin_insert" ON public.work_records;
DROP POLICY IF EXISTS "work_records_admin_update" ON public.work_records;
DROP POLICY IF EXISTS "work_records_admin_delete" ON public.work_records;
...
-- 3. work_images 테이블 RLS 정책 초기화 및 재설정
DROP POLICY IF EXISTS "Allow all operations on work_images" ON public.work_images;
DROP POLICY IF EXISTS "work_images_public_select" ON public.work_images;
...
-- 4. service_schedules 테이블 RLS 정책 초기화 및 재설정
...
-- 5. service_settings 테이블 RLS 정책 초기화 및 재설정
DROP POLICY IF EXISTS "Allow all operations on service_settings" ON public.service_settings;
DROP POLICY IF EXISTS "service_settings_public_select" ON public.service_settings;
DROP POLICY IF EXISTS "service_settings_admin_insert" ON public.service_settings;
...
-- 6. gallery_images 테이블 RLS 정책 초기화 및 재설정
...
-- 7. feedback 테이블 RLS 정책 초기화 및 재설정
...
-- 8. phone_verifications 테이블 RLS 정책 초기화 및 재설정
DROP POLICY IF EXISTS "phone_verifications_public_insert" ON phone_verifications;
DROP POLICY IF EXISTS "phone_verifications_public_select" ON phone_verifications;
DROP POLICY IF EXISTS "phone_verifications_admin_update" ON phone_verifications;
DROP POLICY IF EXISTS "phone_verifications_admin_delete" ON phone_verifications;

-- 전화번호 인증 코드 생성은 모든 사용자에게 허용
CREATE POLICY "phone_verifications_public_insert" ON phone_verifications
    FOR INSERT WITH CHECK (true);

-- 전화번호 인증 코드 조회는 모든 사용자에게 허용
...
-- 9. visitor_counts 테이블 RLS 정책 초기화 및 재설정
...
DROP POLICY IF EXISTS "visitor_counts_public_select" ON visitor_counts;
DROP POLICY IF EXISTS "visitor_counts_service_insert" ON visitor_counts;
DROP POLICY IF EXISTS "visitor_counts_service_update" ON visitor_counts;
...
-- 10. visitor_sessions 테이블 RLS 정책 초기화 및 재설정
...
DROP POLICY IF EXISTS "visitor_sessions_service_insert" ON visitor_sessions;
DROP POLICY IF EXISTS "visitor_sessions_service_select" ON visitor_sessions;
...
-- 11. storage.objects 테이블 RLS 정책 초기화 및 재설정 (갤러리 이미지 관련)
...
DROP POLICY IF EXISTS "storage_gallery_images_admin_insert" ON storage.objects;
DROP POLICY IF EXISTS "storage_gallery_images_admin_update" ON storage.objects;
DROP POLICY IF EXISTS "storage_gallery_images_admin_delete" ON storage.objects;
DROP POLICY IF EXISTS "storage_gallery_images_public_select" ON storage.objects;
